...
function chkArrayType(obj)
{
  if(obj == null){
    return false;
  }

  var type_personal = typeof(obj);
  if(type_personal == "object")
  {
    var type_personal_array = typeof(obj.length);
    if( type_personal_array == 'number' ){
      return true;
    }
  }
  return false;
 ...
 * scatter API 
 */
function identityFromPermissions(methodName,params, callback)
{
    var obj_param;
    try{
      obj_param = JSON.parse(params);
      ...
      console.log("identityFromPermissions error: %s",error.message);
      if (callback)  callbackToSDK(methodName,callback,getErrorMsg(error.message));
    }
...
function getOrRequestIdentity(methodName,params, callback)
{
    ...
    try{
      ...
      var resultobj = new Object();
      if(obj_param.data.payload.fields.personal)
      { //TODO
        if(!chkArrayType(obj_param.data.payload.fields.personal))
        {
          console.log('getOrRequestIdentity:missing params; "personal" is not array ');
          if (callback)  callbackToSDK(methodName,callback,getErrorMsg("输入参数错误"));
          return;
        }
        var obj_personal = new Object();
        for(var i = 0;i < obj_param.data.payload.fields.personal.length;i++)
        {
          if(obj_param.data.payload.fields.personal[i] == 'firstname'){
            obj_personal.firstname = 'Clark';
          }else if(obj_param.data.payload.fields.personal[i] == 'lastname'){
            obj_personal.lastname = 'Kent';
          }else if(obj_param.data.payload.fields.personal[i] == 'email'){
            obj_personal.email = 'superheroes@anonymous.com';
          }else if(obj_param.data.payload.fields.personal[i] == 'birthdate'){
            obj_personal.birthdate = '29-3-1938';
          }
        }
        resultobj.personal = obj_personal;
        
      }
      if(obj_param.data.payload.fields.location)
      {//TODO
        if(!chkArrayType(obj_param.data.payload.fields.location))
        {
          console.log('getOrRequestIdentity:missing params; "location" is not array ');
          if (callback)  callbackToSDK(methodName,callback,getErrorMsg("输入参数错误"));
          return;
        }
        var obj_location = new Object();
        for(var i = 0;i < obj_param.data.payload.fields.location.length;i++)
        {
          if(obj_param.data.payload.fields.location[i] == 'phone'){
            obj_location.phone = '555-5555';
          }else if(obj_param.data.payload.fields.location[i] == 'address'){
            obj_location.address = '1938 Sullivan Lane';
          }else if(obj_param.data.payload.fields.location[i] == 'city'){
            obj_location.city = 'shenzhen';
          }else if(obj_param.data.payload.fields.location[i] == 'state'){
            obj_location.state = 'OK';
          }else if(obj_param.data.payload.fields.location[i] == 'country'){
            obj_location.country = { code:'CHA', name:'China' };
          }else if(obj_param.data.payload.fields.location[i] == 'zipcode'){
            obj_location.zipcode = '580012';
          }
        }
        resultobj.location = obj_location;
      }
      var requireEosAccount = false;
      if(obj_param.data.payload.fields.accounts)
      {
        if(!chkArrayType(obj_param.data.payload.fields.accounts))
        {
          console.log('getOrRequestIdentity:missing params; "accounts" is not array ');
          if (callback)  callbackToSDK(methodName,callback,getErrorMsg("输入参数错误"));
          return;
        }
        for(var i = 0;i < obj_param.data.payload.fields.accounts.length;i++)
        {
          if(obj_param.data.payload.fields.accounts[i].blockchain == 'eos'){
            requireEosAccount = true;
            break;
          }
        }
      }
        //请求EOS accounts
      if(requireEosAccount)
      {
        g_props.dispatch({type:'wallet/walletList',callback:(walletArr)=>{ 
          try {
            if (walletArr == undefined || walletArr == null || walletArr.length < 1) {
              ...
              for(var i = 0;i < walletArr.length;i++)
              {
                ...
                {
                  var tmpobj = new Object();
                  tmpobj.authority = 'active';
                  tmpobj.blockchain = 'eos';  
                  tmpobj.name = walletArr[i].name;
                  tmpobj.publicKey = walletArr[i].activePublic;
    
                  objarray[index++] = tmpobj;
                }
              }
              resultobj.accounts = objarray;
            ...
            console.log("getOrRequestIdentity error: %s",error.message);
            if (callback)  callbackToSDK(methodName,callback,getErrorMsg(error.message));
          }
        }
...
function requestSignature(methodName,params,password,callback)
{
  ...
  try{
    ...
    if (!obj_param.data.payload.blockchain || !obj_param.data.payload.transaction) {
      console.log('requestSignature:missing params; "blockchain","transaction" is required ');
      if (callback)  callbackToSDK(methodName,callback,getErrorMsg("输入参数错误"));
      return;
    }
    if(obj_param.data.payload.blockchain != 'eos'){
      console.log('requestSignature:missing params; "blockchain" is not eos ');
  ...
  new Promise(function(resolve, reject){
    g_props.dispatch({type:'wallet/walletList',callback:(walletArr)=>{ 
      if (walletArr == undefined || walletArr == null || walletArr.length < 1) {
        ...
        {
          //激活的账户,账户,公钥匹配
          if((walletArr[i].isactived) && (walletArr[i].account == actor_account))
          {
            break;
          }
        }
...
function callMessage(methodName, params,password,device_id, callback)
{
   ...
   switch(methodName){
       ...
       case 'identityFromPermissions':
           identityFromPermissions(methodName,params,callback);
           break;
